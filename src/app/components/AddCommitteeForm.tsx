...
interface AddCommitteeFormProps {
  onClose: () => void;
  onSave: (committeeName: string) => void;
}

export function AddCommitteeForm({ onClose, onSave }: AddCommitteeFormProps) {
  const [committeeName, setCommitteeName] = useState('');

  const handleSave = () => {
    if (committeeName.trim()) {
      onSave(committeeName.trim());
      setCommitteeName('');
    }
  };

  const isSaveDisabled = !committeeName.trim();
          ...
          <div className="content-stretch flex flex-[1_0_0] flex-col gap-[8px] items-start min-h-px min-w-px relative">
            <div className="content-stretch flex flex-col items-start relative shrink-0 w-full">
              <div className="flex flex-col font-semibold leading-[0] min-h-[32px] not-italic relative shrink-0 text-[#041a5e] text-[20px] w-full">
                <p className="leading-[1.3]">Add New Committee</p>
              </div>
            </div>
          </div>
          <button 
