...
import { AddParentInstitutionForm } from '../components/AddParentInstitutionForm';
...
export default function GlobalListsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'parent-institutions' | 'institutions' | 'ministries' | 'committees'>('parent-institutions');
  ...
  const [isAddInstitutionFormOpen, setIsAddInstitutionFormOpen] = useState(false);
  const [isAddParentInstitutionFormOpen, setIsAddParentInstitutionFormOpen] = useState(false);
  ...
  const filteredParentInstitutions = parentInstitutions.filter(parentInstitution =>
    parentInstitution.name.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const handleAddInstitution = (institutionName: string, institutionType: string, parentInstitutionId?: number) => {
    addInstitution(institutionName, institutionType, parentInstitutionId);
    setIsAddInstitutionFormOpen(false);
  };

  const handleAddParentInstitution = (parentInstitutionName: string) => {
    addParentInstitution(parentInstitutionName);
    setIsAddParentInstitutionFormOpen(false);
  };
  ...
  const isInstitutionPanelOpen = activeTab === 'institutions' && isAddInstitutionFormOpen;
  const isParentInstitutionPanelOpen = activeTab === 'parent-institutions' && isAddParentInstitutionFormOpen;
  const isAnyPanelOpen = isMinistryPanelOpen || isCommitteePanelOpen || isInstitutionPanelOpen || isParentInstitutionPanelOpen;
    ...
    <div className="bg-[#f8f9fb] relative size-full" data-name="Global Lists">
      ...
      <div className="absolute content-stretch flex flex-col gap-[16px] items-start page-inset-x top-[100px]">
        ...
        <div className="flex gap-[12px] items-start">
          <button
            onClick={() => setActiveTab('parent-institutions')}
            className="flex items-center p-[12px] relative"
          >
            {activeTab === 'parent-institutions' && (
            ...
            <p className={`font-semibold leading-[16px] text-[length:var(--text-base)] text-center relative ${
              activeTab === 'parent-institutions' ? 'text-[var(--accent)]' : 'text-[var(--muted-foreground)]'
            }`}>
              Parent Institutions
            </p>
          </button>
          ...
          >
            ...
            <p className={`font-semibold leading-[16px] text-[length:var(--text-base)] text-center relative ${
              activeTab === 'institutions' ? 'text-[var(--accent)]' : 'text-[var(--muted-foreground)]'
            }`}>
              Institutions
            </p>
          </button>
        ...
        <div className="flex items-center justify-between w-full">
          <div className="w-[320px]">
            <SearchField 
              placeholder={activeTab === 'ministries' ? 'Search Ministries' : activeTab === 'committees' ? 'Search Committee' : activeTab === 'institutions' ? 'Search Institution' : 'Search Parent Institution'} 
              value={searchQuery}
          ...
          <PrimaryActionButton 
            label={activeTab === 'ministries' ? 'New Ministry' : activeTab === 'committees' ? 'New Committee' : activeTab === 'institutions' ? 'New Institution' : 'New Parent Institution'} 
            onClick={() => {
              if (activeTab === 'ministries') {
                ...
                setIsAddInstitutionFormOpen(true);
              } else if (activeTab === 'parent-institutions') {
                setIsAddParentInstitutionFormOpen(true);
              }
            }}
          />
        </div>
        ...
        <div className="content-stretch flex gap-[16px] items-start relative shrink-0 w-full">
          ...
          <div className={`content-stretch flex flex-col gap-[8px] items-start relative shrink-0 max-h-[calc(100vh-230px)] overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden ${
              ...
              >
                ...
                <div className="flex flex-row items-center size-full">
                  <div className="content-stretch flex gap-[16px] items-center p-[24px] relative w-full">
                    ...
                    <div className="content-stretch flex flex-[1_0_0] gap-[8px] items-center min-h-px min-w-px relative">
                      <div className="flex flex-[1_0_0] flex-col font-semibold justify-center leading-[0] min-h-px min-w-px not-italic overflow-hidden relative text-[var(--foreground)] text-[length:var(--text-base)] text-ellipsis whitespace-nowrap">
                        <p className="leading-[16px] overflow-hidden">{institution.name}</p>
                      </div>
                      <div className="bg-[#e7f2fe] px-[8px] py-[4px] rounded-[var(--radius)] shrink-0">
                        <p className="leading-[14px] text-[#1850c5] text-[12px]">{institution.type}</p>
                      </div>
                    </div>
            ...
            {activeTab === 'parent-institutions' && filteredParentInstitutions.map((parentInstitution) => (
              <div
                key={parentInstitution.id}
                className="bg-[var(--card)] relative rounded-[var(--radius-card)] shrink-0 w-full"
              >
                <div aria-hidden="true" className="absolute border border-[var(--card-border)] border-solid inset-0 pointer-events-none rounded-[var(--radius-card)]" />
                <div className="flex flex-row items-center size-full">
                  <div className="content-stretch flex gap-[16px] items-center p-[24px] relative w-full">
                    ...
                    <div className="content-stretch flex flex-[1_0_0] gap-[8px] items-center min-h-px min-w-px relative">
                      <div className="flex flex-[1_0_0] flex-col font-semibold justify-center leading-[0] min-h-px min-w-px not-italic overflow-hidden relative text-[var(--foreground)] text-[length:var(--text-base)] text-ellipsis whitespace-nowrap">
                        <p className="leading-[16px] overflow-hidden">{parentInstitution.name}</p>
                      </div>
                    </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {isAddInstitutionFormOpen && activeTab === 'institutions' && (
              <motion.div 
                className="content-stretch flex flex-col items-start relative shrink-0 w-[416px]"
                initial={{ x: '100%', opacity: 0 }}
                animate={{ x: 0, opacity: 1 }}
              ...
              >
                <AddInstitutionForm 
                  onClose={() => setIsAddInstitutionFormOpen(false)}
                  onSave={handleAddInstitution}
                  parentInstitutions={parentInstitutions}
                />
              </motion.div>
            )}
            {isAddParentInstitutionFormOpen && activeTab === 'parent-institutions' && (
              <motion.div 
                className="content-stretch flex flex-col items-start relative shrink-0 w-[416px]"
                initial={{ x: '100%', opacity: 0 }}
                animate={{ x: 0, opacity: 1 }}
                ...
                transition={{ type: 'spring', stiffness: 300, damping: 30 }}
              >
                <AddParentInstitutionForm 
                  onClose={() => setIsAddParentInstitutionFormOpen(false)}
                  onSave={handleAddParentInstitution}
                />
              </motion.div>
            )}
